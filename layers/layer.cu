...
{
    feature_transformer_slice_forward<<<BATCH_SIZE, FT_BLOCK_SIZE>>>(feature_indices_us, layer->weights->data, layer->biases->data, layer->output.unactivated->data);
    cudaDeviceSynchronize();
    feature_transformer_slice_forward<<<BATCH_SIZE, FT_BLOCK_SIZE>>>(feature_indices_enemy, layer->weights->data, layer->biases->data, &layer->output.unactivated->data[L1]);
    cudaDeviceSynchronize();
...
void backwardFeatureTransformer(int32_t *feature_indices_us, int32_t *feature_indices_enemy, Layer *layer)
{
    ...
    feature_transformer_slice_backward<<<BATCH_SIZE, FT_BLOCK_SIZE>>>(feature_indices_us, layer->weight_gradients->data, layer->bias_gradients->data, layer->output.gradients->data);
    cudaDeviceSynchronize();
    feature_transformer_slice_backward<<<BATCH_SIZE, FT_BLOCK_SIZE>>>(feature_indices_enemy, layer->weight_gradients->data, layer->bias_gradients->data, &layer->output.gradients->data[L1]);
    cudaDeviceSynchronize();
}
void forwardLinear(Output previousLayerOutput, Layer *layer)
{
    MultipyMatrix(previousLayerOutput.activated, layer->weights, layer->output.unactivated, 0, 0);
...
void backwardLinear(Output previousLayerOutput, Layer *layer)
{
    ...
    MultipyMatrix(layer->output.gradients, layer->weights, previousLayerOutput.gradients, 0, 1);
    MultipyMatrix( previousLayerOutput.activated, layer->output.gradients, layer->weight_gradients, 1, 0);
}
